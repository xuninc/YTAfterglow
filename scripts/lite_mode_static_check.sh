...
require 'YTAGLiteThemeBackupValuesKey' 'Utils/YTAGLiteMode.m' 'Lite Mode must save theme backup values before applying its preset'
require 'YTAGLiteThemeBackupMissingKeysKey' 'Utils/YTAGLiteMode.m' 'Lite Mode must remember missing theme keys for exact restore'
require 'YTAGLiteModeRestoreThemeBackupIfNeeded' 'Utils/YTAGLiteMode.m' 'Lite Mode must restore the previous theme when disabled'
...
for marker in community shopping breakingnews mixplaylist radio chipcloud filterchip promoted sponsor commerce reel shortsshelf richshelf suggestedvideo watchnext shoppingitem shoppingcompanion merchandise storeshelf storebutton; do
  require "$marker" 'Utils/YTAGLiteMode.m' "Lite feed pruning must include $marker surfaces"
done

for compact_symbol in YTAGLiteModeCompactFeedVideoWidthKey YTAGLiteModeCompactFeedVideoScale YTAGLiteModeApplyCompactFeedLayout YTAGLiteModeApplyCompactFeedPlaybackLayout YTAGLiteCompactFeedCandidateScore YTAGLiteBestCompactFeedThumbnailCandidate LiteCompactFeedVideoWidth; do
  if rg -q "$compact_symbol" "$ROOT/Utils/YTAGLiteMode.h" "$ROOT/Utils/YTAGLiteMode.m" "$ROOT/YTAfterglow.x" "$ROOT/Settings.x" "$ROOT/Utils/YTAGUserDefaults.m" "$ROOT/layout/Library/Application Support/YTAfterglow.bundle/en.lproj/Localizable.strings"; then
    echo "FAIL: Lite Mode must use standard native feed sizing; found retired compact-feed symbol $compact_symbol" >&2
    exit 1
  fi
done

if perl -0ne 'exit(/%hook YTElementsInlineMutedPlaybackView|%hook YTInlineMutedPlaybackWatchView|%hook YTInlineMutedPlaybackPlayerOverlayView\b/s ? 0 : 1)' "$ROOT/YTAfterglow.x"; then
  echo "FAIL: Lite Mode must not hook inline playback views to resize native feed video surfaces" >&2
  exit 1
fi
...
if rg -q 'width = MIN\(MAX\(width > 0 \? width : 88, 80\), 100\)' "$ROOT/Utils/YTAGLiteMode.m"; then
  echo "FAIL: retired Lite feed sizing code must not return to the old 80-100 percent range" >&2
  exit 1
fi
