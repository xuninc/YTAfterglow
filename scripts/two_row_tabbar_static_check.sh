...
require 'ytagBool\(@"twoRowTabBar"\) \? ytag_allTabIds\(\)\.count : 6' 'Settings.x' 'two-row tab editor limit must allow all tabs in bonus-row mode and six in normal mode'
...
require '"Tabbar" = "Tab bar"' 'layout/Library/Application Support/YTAfterglow.bundle/en.lproj/Localizable.strings' 'Settings page title must remain Tab bar'
...
require '@\[@"FEhype_leaderboard", @"FEhistory", @"VLWL", @"FEpost_home"\]' 'YTAfterglow.x' 'only real browse tabs may be synthesized'
require 'return !\[tabId isEqualToString:@"FEuploads"\]' 'YTAfterglow.x' 'Create must not be synthesized as a fake browse tab'
require 'pivotBarIconOnlyItemRenderer.*pivotIdentifier' 'YTAfterglow.x' 'Create must be matched through its native icon-only renderer'
...
require 'YTAGThemeTabLabelSizeModeKey' 'Settings.x' 'Themes Typography must expose tab label size'
require 'TabLabelSize' 'layout/Library/Application Support/YTAfterglow.bundle/en.lproj/Localizable.strings' 'tab label size copy must be localized'
require 'ytagApplyNativeTabLabelTypography' 'YTAfterglow.x' 'native tab labels must get a dedicated typography fit pass'
require 'label\.numberOfLines = 1' 'YTAfterglow.x' 'native tab labels must stay single-line'
require 'label\.adjustsFontSizeToFitWidth = YES' 'YTAfterglow.x' 'native tab labels must shrink instead of wrapping'
require 'kYTAGNativeTabLabelBaseFontKey' 'YTAfterglow.x' 'native tab label shrinking must keep a stable base font'
require 'kYTAGNativeTabLabelAppliedFontKey' 'YTAfterglow.x' 'native tab label shrinking must not shrink repeatedly on layout'
...
reject '@\[@"FEhype_leaderboard", @"FEhistory", @"VLWL", @"FEpost_home", @"FEuploads"\]' 'YTAfterglow.x' 'Create must not be added to the synthesized browse tab list'
reject 'ytagBool\(@"twoRowTabBar"\) \? \[YTAGUserDefaults defaultActiveTabs\]\.count : 6' 'Settings.x' 'two-row tab editor limit must count all available tabs, not the four default tabs'
